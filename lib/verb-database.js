...
export async function getVerbs(count, difficulty, user) {
    ...
    if (user) {
        ...
        if (seenCount === difficulty) {
            ...
            result.forEach(v => {
                v.consecutive = infoMap[v.uid].consecutive
            })
        }
        ...
        let useUnseen = Math.min(~~(0.8 * count), unseenCount)
        let useSeen = count - useUnseen
        const mistakeDiff = useSeen - mistakeCount
        if(mistakeDiff > 0) {
            useSeen -= mistakeDiff
            useUnseen += mistakeDiff
        }
...
export const VERB_COUNT = allVerbs.length
